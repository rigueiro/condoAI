...
import type { CondoRow, SortConfig } from "../types";
            ...
            <tr>
              <th className="px-6 py-4 text-left">
                <input
                  type="checkbox"
                  checked={
                    selectedProperties.length === rows.length && rows.length > 0
                  }
            ...
            {rows.map(({ condo, stats }) => {
              ...
              const occupancy =
                condo.numberOfUnits > 0
                  ? Math.round((stats.occupiedUnits / condo.numberOfUnits) * 100)
                  : 0;
              ...
              return (
                ...
                >
                  <td className="px-6 py-4">
                    <input
                      type="checkbox"
                      checked={selectedProperties.includes(condo.id)}
                      onChange={() => onSelectProperty(condo.id)}
                      className="w-4 h-4 text-primary border-secondary-300 rounded focus:ring-primary focus:ring-2"
                    />
                  </td>
                  <td className="px-6 py-4">
                    <div>
                      <Link
                        href={`/properties-management/${condo.id}`}
                  ...
                  <td className="px-6 py-4">
                    ...
                    <div className="text-xs text-text-secondary">
                      {t("avgFee", {
                        amount: formatCurrency(stats.averageFee),
                      })}
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div
                      className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${getCollectionRateBg(stats.collectionRate)} ${getCollectionRateColor(stats.collectionRate)}`}
                    >
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  <span
                    className={`ml-2 font-medium ${getCollectionRateColor(stats.collectionRate)}`}
                  >
                    {stats.collectionRate}%
                  </span>
                </div>
