...
function FinanceModal({
  ...
  const [year, setYear] = useState(
    record?.kind === "budget"
      ? String(record.data.year)
      : String(new Date().getFullYear()),
  );
  const [status, setStatus] = useState<AnnualBudget["status"]>(
    record?.kind === "budget" ? record.data.status : "draft",
  );
  const [categoriesText, setCategoriesText] = useState(
    record?.kind === "budget"
      ? categoriesToText(record.data.valuesByCategory)
      : "cleaning: 0\nelectricity: 0\ninsurance: 0\nmaintenance: 0\nreserve: 0",
  );
  ...
  const validate = (): boolean => {
    ...
    if (kind === "budget") {
      ...
      if (!parseCategories(categoriesText)) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (kind === "budget") {
      const valuesByCategory = parseCategories(categoriesText);
      if (!valuesByCategory) return;
      onSave({
        kind,
        data: {
          id,
