...
import {
  OPERATIONS_FILE_ACCEPT,
  isAllowedOperationsFile,
...
function DocumentUpload({
  ...
  const t = useTranslations("operations.modal.file");
  ...
  const handleChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    const validation = isAllowedOperationsFile(file);
  ...
  const handleOpen = () => {
    if (!value) return;
    openOperationsDocument(value);
  };

  const attachedLabel = operationsFileLabel(value);
